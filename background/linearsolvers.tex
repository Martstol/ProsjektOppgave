...
From the section on the Jacobi method, we have the expression 
...
However as we don't know the exact solution, for convergence we use the $k$th residual 
vector
...
system with $I$ as the diagonal matrix, $I = D$. If we solve this system instead we 
get
...
$$ x^{(n)} = x^{(0)} + r^{(0)} + ... + r^{(n-1)} = x^{(0)} + q_{n-1}(A)r^{(0)} $$
