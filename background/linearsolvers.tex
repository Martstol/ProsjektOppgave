...
number of steps. Two of well known methods are Gaussian elimination, and LU factorization.
...
LU factorization also has computational complexity O($n^3$), however the constant 
factors of the LU factorization means that it requires half the number of floating point operations
compared to Gaussian elimination, and forward and backward substitution is O($n^2$)\cite{Kreyszig}.
