...
trace the particles path through the velocity field. If $x$ is a point in the
three dimensional domain, the fluid's velocity at that point for time step $n$
is given by $u(x)_n$. The velocity of the particle at position $x$ is calculated
by backtracking through the velocity field as illustrated in figure .

$$ u^*(x) = u(x - u(x)_n \delta t) $$
